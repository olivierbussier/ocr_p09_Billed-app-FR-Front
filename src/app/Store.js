...
const getHeaders = (headers) => {
    const h = { }
    if (!headers.noContentType) {
        h['Content-Type'] = 'application/json'
    }
    const jwt = localStorage.getItem('jwt')
    if (jwt && !headers.noAuthorization) {
...
class Store {
    ...
    user = (uid) => {
        return (new ApiEntity({key: 'users', api: this.api})).select({selector: uid})
    }

    users = () => {
        return new ApiEntity({key: 'users', api: this.api})
    }
    ...
    bill = bid => {
        return (new ApiEntity({key: 'bills', api: this.api})).select({selector: bid})
    }
